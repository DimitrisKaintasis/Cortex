...
class OllamaTemporalSummarizer(TemporalSummarizer):
    """Evidence-grounded Temporal History summarization through Ollama."""
    ...
    def _summarize(
        ...
        while len(self._json(current)) > self.max_input_chars:
            ...
            if len(self._json(next_items)) >= len(self._json(current)):
                raise OllamaError("Temporal input could not be reduced within the model limit")
            current = next_items
    ...
    def _groups(self, items: list[dict[str, Any]]) -> list[list[dict[str, Any]]]:
        ...
        for item in items:
            if len(self._json([item])) > self.max_input_chars:
                raise OllamaError("one Temporal input item exceeds the model input limit")
    ...
    def _system_prompt() -> str:
        return (
            ...
            "come only from non-null thread_root_id or child thread_references fields; an event "
            "ID or source message ID is not a thread reference. Artefact references must come "
            "only from explicit payload or child artefact-reference fields. Use an empty array "
            "when evidence for a field is absent. "
            "Return exactly one JSON object with keys summary_text, topics, decisions, actions, "
            "open_questions, participants, thread_references, artefact_references, coverage_gaps. "
            "All keys except summary_text contain arrays of strings. Be concise and add no "
            "Markdown."
        )
